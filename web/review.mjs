...
export class Review {
 ...
 draw(){
  ...
  if(this.waitKey!==s){clearTimeout(this.waitTimer);this.waitKey=s;this.slow=false;if(waiting){const epoch=this.epoch;this.waitTimer=setTimeout(()=>{if(epoch!==this.epoch)return;this.slow=true;this.loading(false);this.message('This is taking longer than expected. Video assignment may be temporarily unavailable. You can return to Home; your request is saved and the video will appear here when it is ready.');},30000);this.waitTimer.unref?.();}}
  ...
  if(!this.slow)this.message(texts[s||'idle']||'');
  if(s==='assigned' && j.playbackURL!==this.src){
   ...
   this.loadingVideo=true;this.loading(true);this.src=j.playbackURL;this.el('review-video').src=this.src;
  }
  if(!['assigned','refresh_requested'].includes(s)){this.el('review-video').pause();this.el('review-video').removeAttribute('src');this.src=null;this.loadingVideo=false;}
  ...
  this.el('review-outcome').textContent=({approved:'Review approved',test_only:'Non-qualifying review saved',ineligible:'Review not eligible',report_confirmed:'Report confirmed · Video removed',report_not_upheld:'Report reviewed · Video retained',adjudication_required:'Needs a closer look'})[outcome?.status]||'Awaiting consensus';
  this.el('review-progress').textContent=outcome?.status==='report_confirmed'?'Independent reports confirmed the video was unusable. Thank you for helping protect the corpus.':outcome?.status==='report_not_upheld'?'Independent interpretations supported this video. Your original report is preserved.':outcome?.status==='approved'?'5 approved points earned.':outcome?.status==='test_only'?'Saved, but this is not an independent review and earns no points.':outcome?.status==='adjudication_required'?'A human check is needed. No points awarded for this review.':`${outcome?.independentReviews||0} of ${outcome?.requiredReviews||3} independent reviews received.`;this.buttons();
 }
